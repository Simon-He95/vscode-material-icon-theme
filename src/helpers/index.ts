...
export const getConfigProperties = (): { [config: string]: unknown } => {
  return extensions.getExtension('simonhe.material-icon-theme-simon')
...
export const getThemeConfig = (section: string) => {
  return getConfig('material-icon-theme-simon').inspect(section);
};
...
export const setThemeConfig = (
  ...
  return getConfig('material-icon-theme-simon').update(section, value, global);
};
...
const getExtensionPath = () =>
  extensions.getExtension('simonhe.material-icon-theme-simon')?.extensionPath ??
  '';
